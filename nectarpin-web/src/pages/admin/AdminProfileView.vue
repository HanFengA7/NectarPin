...
async function loadSessions() {
  ...
  try {
    ...
    sessions.value = []
    sessionsError.value = e instanceof RequestError ? e.message : '无法加载登录设备列表'
  } finally {
...
onMounted(async () => {
  ...
  try {
    ...
    loadError.value = e instanceof RequestError ? e.message : '无法加载个人资料'
  } finally {
    loading.value = false
  }
...
async function handleRevokeSession(item: UserSessionItem) {
  ...
  try {
    ...
    sessionsError.value = e instanceof RequestError ? e.message : '操作失败'
  } finally {
...
async function handleSaveProfile() {
  ...
  try {
    ...
    profileError.value = e instanceof RequestError ? e.message : '保存失败，请稍后重试。'
  } finally {
...
async function handleChangePassword() {
  ...
  try {
    ...
    passwordError.value = e instanceof RequestError ? e.message : '修改密码失败，请稍后重试。'
  } finally {
    ...
    <div class="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
      ...
      >
        <div class="flex items-start gap-4">
          ...
          <div>
            <h1 class="text-2xl font-semibold tracking-tight">个人资料与账号安全</h1>
      ...
      <div v-else class="space-y-6 lg:space-y-8">
        <div class="grid gap-6 lg:grid-cols-2 lg:items-start lg:gap-8">
          ...
          <div class="rounded-2xl border bg-background p-6 shadow-sm sm:p-8">
            ...
            <form class="space-y-5" @submit.prevent="handleSaveProfile">
              ...
              <Button type="submit" class="w-full sm:w-auto" :disabled="savingProfile">
                <LoaderCircle v-if="savingProfile" class="size-4 animate-spin" />
          ...
          <div class="rounded-2xl border bg-background p-6 shadow-sm sm:p-8">
            ...
            <p class="mb-5 text-sm text-muted-foreground">新密码至少 6 位字符。</p>
        ...
        <div class="rounded-2xl border bg-background p-6 shadow-sm sm:p-8">
          <div
            class="mb-4 flex flex-col gap-4 border-b border-border/80 pb-4 sm:flex-row sm:items-center sm:justify-between"
          >
            <div class="flex items-center gap-2">
              <MonitorSmartphone class="size-5 text-muted-foreground" />
              <h2 class="text-lg font-semibold">登录设备</h2>
            </div>
            <Button
            ...
            >
              <LoaderCircle v-if="revokingOthers" class="size-4 animate-spin" />
            ...
            <table class="w-full min-w-[520px] text-left text-sm">
              <thead class="border-b bg-muted/40 text-muted-foreground">
                <tr>
                  ...
                  <th class="px-4 py-3 font-medium">登录时间</th>
                  <th class="px-4 py-3 font-medium">过期时间</th>
                  <th class="w-32 px-4 py-3 font-medium">操作</th>
                </tr>
              </thead>
              <tbody>
                <tr
                ...
                >
                  ...
                  <td class="px-4 py-3">
                    ...
                    >
                      <LoaderCircle v-if="revokingId === s.id" class="size-4 animate-spin" />
