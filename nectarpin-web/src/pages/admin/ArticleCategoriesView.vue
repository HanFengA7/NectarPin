...
async function handleDelete(item: ArticleCategoryItem) {
  const confirmed = window.confirm(
    `确定删除分类「${item.name}」吗？该分类下的文章将变为"未分类"。`,
  )
  if (!confirmed) return
  ...
  <section class="min-h-svh bg-muted/20">
    <div class="flex w-full flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8">
      ...
      <section class="rounded-2xl border bg-background shadow-sm">
        ...
        <template v-else>
          <div v-if="categories.length === 0" class="flex min-h-64 flex-col items-center justify-center gap-3 px-6 py-10 text-center">
            <div class="space-y-1">
            ...
            <table class="min-w-full text-sm">
              ...
              <tbody>
                ...
                >
                  ...
                  <td class="max-w-xs truncate px-6 py-4 text-muted-foreground">{{ item.description || '-' }}</td>
